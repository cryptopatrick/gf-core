...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Available languages: {:?}",
        grammar.concretes.keys().collect::<Vec<_>>()
    );
    ...
    loop {
        ...
        if translations.is_empty() {
            ...
            for (lang_code, concrete) in &grammar.concretes {
                let completion = concrete
                    .complete(input, &grammar.abstract_grammar.startcat);
                if !completion.suggestions.is_empty() {
                    println!(
                        "  {} suggestions: {:?}",
                        lang_code, completion.suggestions
                    );
                }
            }
        } else {
            for (source_idx, translation_map) in
                translations.iter().enumerate()
            {
